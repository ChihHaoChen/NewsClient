...
class TodayController: BaseCollectionViewController, UICollectionViewDelegateFlowLayout, UIGestureRecognizerDelegate   {
    ...
    func fetchAPI() {
        ...
        dispatchGroup.notify(queue: .main)  {
            self.items = [
                TodayItem.init(category: "THE DAILY LIST", title: "News" ?? "", image: #imageLiteral(resourceName: "holiday"), description: "All the New you are eager to know right way", backgroundColor: .white, cellType: .multiple, newsFetch: self.businessUS?.articles ?? []),
                TodayItem.init(category: "THE DAILY LIST", title: "News" ?? "", image: #imageLiteral(resourceName: "holiday"), description: "All the New you are eager to know right way", backgroundColor: .white, cellType: .multiple, newsFetch: self.businessJapan?.articles ?? [])
            ]
